...
> 应用名：**翡栖** ｜ 包名：`com.feiqi` ｜ 版本：**v1.5.1** (versionCode 64)
...
| **自动收集集** | [一言](https://hitokoto.cn)「文学 + 诗词」类别随机取句 | **每周一首次打开 App** 静默收集 **5~10 条**，去重后追加（上限 400 条） |

- **全自动、无入口**：设置里没有手动更新开关。触发条件是**周一 + 本周尚未更新**（其余时间打开 App 不做任何请求），失败静默（只写日志）、不打扰用户；周一若联网失败（一条都没取到），本周不算完成——当天稍晚再打开（间隔 ≥3 小时）会自动重试。
