...
> 应用名：**翡栖** ｜ 包名：`com.feiqi` ｜ 版本：**v1.5.0** (versionCode 63)
...
| **自动收集集** | [一言](https://hitokoto.cn)「文学 + 诗词」类别随机取句 | **每次打开 App** 静默收集，去重后追加（每轮 5 条，上限 400 条） |

- **全自动、无入口**：设置里没有手动更新开关——App 每次进入前台自动完成，失败静默（只写日志），不打扰用户。
